...
class TestCpmHubConnectorV1(unittest.TestCase):
    ...
    def test_publish_bit_posts_bit_with_base64_encoded_payload_and_user_credentials(self, getpass, input, http_client):
    ...
    @patch('builtins.input')
    @patch('cpm.infrastructure.cpm_hub_connector_v1.getpass')
    def test_publish_bit_raises_authentication_error_on_status_code_401(self, getpass, input, http_client):
        filesystem = MagicMock()
        project = Project('cpm-hub')
        connector = CpmHubConnectorV1(filesystem)
    ...
    @patch('builtins.input')
    @patch('cpm.infrastructure.cpm_hub_connector_v1.getpass')
    def test_publish_bit_raises_invalid_cpm_hub_url_on_status_code_404(self, getpass, input, http_client):
        filesystem = MagicMock()
        project = Project('cpm-hub')
        connector = CpmHubConnectorV1(filesystem)
    ...
    @patch('builtins.input')
    @patch('cpm.infrastructure.cpm_hub_connector_v1.getpass')
    def test_publish_bit_raises_publication_error_when_status_code_is_not_200(self, getpass, input, http_client):
        filesystem = MagicMock()
        project = Project('cpm-hub')
        connector = CpmHubConnectorV1(filesystem)
